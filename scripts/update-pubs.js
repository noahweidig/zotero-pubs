...
<script>
(() => {
  const nodes = [...document.querySelectorAll('.type-heading, .entry')];
  const groups = [];
  let cur = null;
  nodes.forEach(el => {
    if (el.classList.contains('type-heading')) { cur = { h: el, es: [] }; groups.push(cur); }
    else if (cur) cur.es.push(el);
  });
  ...
  if (q) q.addEventListener('input', () => {
    ...
    groups.forEach(g => {
      const any = g.es.some(e => e.style.display !== 'none');
      g.h.classList.toggle('is-hidden', !any);
    });
  });
